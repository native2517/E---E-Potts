 ...
 * @see theme_breadcrumb(), eepotts_theme();
 *
 * Changed breadcrumb separator to an image and add current page's title to end
 */
function eepotts_breadcrumb($breadcrumb) {
    if (!empty($breadcrumb)) {
        $separator = theme('image', drupal_get_path('theme', 'eepotts') . '/images/black-bullet.gif');
 ...
 * @ingroup themeable
 */
function eepotts_forum_icon($new_posts, $num_posts = 0, $comment_mode = 0, $sticky = 0) {
 ...
 *
 * @see theme_fieldset(), eepotts_theme(), eepotts.js
 *
 * Removed drupal js and label and created a title div
 */
function eepotts_fieldset($element) {
 ...
 */
function eepotts_id_safe($string) {
 ...
 */
function eepotts_conditional_stylesheets() {
 ...
 * @return string
 */
function eepotts_render_attributes($attributes) {
    if ($attributes) {
        $items = array();
 ...
 * @return Array
 */
function eepotts_preprocess(&$vars, $hook) {
    if (is_file(drupal_get_path('theme', 'eepotts') . '/preprocess/preprocess-' . str_replace('_', '-', $hook) . '.inc')) {
